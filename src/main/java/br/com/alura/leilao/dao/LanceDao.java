package br.com.alura.leilao.dao;

import javax.persistence.EntityManager;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

import br.com.alura.leilao.model.Lance;
import br.com.alura.leilao.model.Leilao;

@Repository
public class LanceDao {

	private EntityManager em;
	
	@Autowired
	public LanceDao(EntityManager em) {
		this.em = em;
	}

	public void salvar(Lance lance) {
		 this.em.persist(lance);
	}

	public Lance buscarMaiorLanceDoLeilao(Leilao leilao) {
		return em.createQuery("SELECT l FROM Lance l WHERE l.valor = (SELECT MAX(lance.valor) FROM Lance lance WHERE lance.leilao = :leilao)", Lance.class)
				.setParameter("leilao", leilao)
				.getSingleResult();
	}
	
	public String buscarMenorLanceDoLeilao(Leilao leilao) {
		return em.createQuery("SELECT l.usuario.nome FROM Lance l WHERE l.valor = (SELECT MIN(lance.valor) FROM Lance lance WHERE lance.leilao = :leilao)", String.class)
				.setParameter("leilao", leilao)
				.getSingleResult();
	}

	public Lance buscarPorId(Long id) {
		return em.find(Lance.class, id);
	}
	
	public Lance buscarPorUsername(String username) {
		return em.createQuery("SELECT l FROM Lance l WHERE l.usuario.nome = :username", Lance.class)
				.setParameter("username", username)
				.getSingleResult();
	}
	
	public Leilao buscarPorUsernameLeilao(String username) {
		return em.createQuery("SELECT l FROM Leilao l WHERE l.usuario.nome = :username", Leilao.class)
				.setParameter("username", username)
				.getSingleResult();
	}
}
